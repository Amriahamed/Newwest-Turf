document.addEventListener('DOMContentLoaded', () => {
  ...
  if (portfolioGrid) {
    ...
    const initialCount = window.innerWidth <= 768 ? 6 : 9;
    ...
    const revealPortfolioItems = () => {
      ...
      if (loadMoreButton) {
        loadMoreButton.style.display = visibleCount >= portfolioItems.length ? 'none' : 'inline-flex';
      }
    };
    ...
    loadMoreButton?.addEventListener('click', () => {
      visibleCount = Math.min(visibleCount + (window.innerWidth <= 768 ? 4 : 6), portfolioItems.length);
      revealPortfolioItems();
    });

    window.addEventListener('resize', () => {
      const newInitialCount = window.innerWidth <= 768 ? 6 : 9;
